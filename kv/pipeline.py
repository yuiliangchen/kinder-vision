...
from kv import edu_advisor, face_insight, identity, llm_edu, macro_analytics, metrics_checker, micro_analytics
from kv import student_longitudinal
from kv.report_pdf import export_combined_report_pdf
from kv.paths import memory_dir, metrics_dir, tmp_dir
from kv.timecode import format_mmss, parse_timecode
from kv.video_ingest import load_audio_mono, read_video_meta
...
def run_full_pipeline(
    ...
    if t0 is not None or t1 is not None:
        from kv.segment import export_video_segment
    ...
    (memory_dir() / f"{date_s}-kinder-macro.md").write_text(
        "# 巨觀層分析（機讀摘要）\n\n```json\n"
    ...
    (memory_dir() / f"{date_s}-kinder-micro.md").write_text(
        "# 微觀層分析（機讀摘要）\n\n```json\n"
    ...
    (memory_dir() / f"{date_s}-kinder-metrics.md").write_text(
        "# 指標核查（機讀摘要）\n\n```json\n"
    ...
    (memory_dir() / f"{date_s}-kinder-edu-report.md").write_text(edu_md, encoding="utf-8")
    ...
    consolidated = [
        ...
        "",
        "*詳見 tmp/kinder-*、memory/ 同日彙總，以及 memory/metrics/ 個別孩童 metrics。*",
    ]
    analysis_path = memory_dir() / f"{date_s}-kinder-analysis.md"
    analysis_path.write_text("\n".join(consolidated), encoding="utf-8")

    pdf_tmp_out: Path | None = None
    pdf_mem_out: Path | None = None
    if emit_pdf:
        pdf_tmp = td / "kinder-report.pdf"
        pdf_mem = memory_dir() / f"{pdf_stamp}-kinder-report.pdf"
        export_combined_report_pdf(analysis_path, td / "kinder-edu-report.md", pdf_tmp)
        pdf_mem.write_bytes(pdf_tmp.read_bytes())
        pdf_tmp_out = pdf_tmp
        pdf_mem_out = pdf_mem
    ...
    out: dict[str, Path] = {
        ...
        "memory_edu": memory_dir() / f"{date_s}-kinder-edu-report.md",
        "memory_analysis": analysis_path,
        "heatmap_png": Path(macro.get("heatmap_png", heatmap_path)),
    }
    if segment_path is not None:
        out["segment_video"] = segment_path
    if pdf_tmp_out is not None and pdf_mem_out is not None:
        out["report_pdf"] = pdf_tmp_out
        out["memory_report_pdf"] = pdf_mem_out
    return out
