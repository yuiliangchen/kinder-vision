...
- `cluster_summary` 提供 `raw_tracks` / `merged_identities` / `adults_excluded` / `child_clusters_total` / `children_kept` / `short_track_dropped`，供下游追蹤收斂效果；detail 見 `docs/agents/identity.md`。
