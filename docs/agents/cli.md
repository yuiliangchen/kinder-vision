...
8. 寫入 `tmp/`、`reports/`（含 `reports/metrics/`）、`memory/students/.../sessions.jsonl`，可選輸出 PDF。
...
- 身分與長期 sessions：`memory/identity_features.db.json`、`memory/students/<student_id>/sessions.jsonl`
