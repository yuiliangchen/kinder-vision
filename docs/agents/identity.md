...
- **`reid_by_track` 與 `cluster_summary`**：輸出仍以「身分 (cluster root)」為單位。`cluster_summary` 欄位：
  - `raw_tracks`：ByteTrack 原始軌跡數
  - `merged_identities`：face ReID 合併後的身分數（含成人）
  - `adults_excluded`：判定為成人並排除的身分數
  - `child_clusters_total`：排除成人後保留的小孩身分數（含軌跡太短者）
  - `children_kept`：實際進入報告的小孩數（`children` list 長度）
  - `short_track_dropped`：因 frame 不足未产出指標的小孩身分數。
